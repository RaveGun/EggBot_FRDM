...
#define isprint(a) (TRUE)

const SerialCommand::SerialCommandCallback commList[] =
{
		{"v",  sendVersion},
		{"EM", enableMotors},
		...
		{"SE", ignore},
		{"TP", togglePen},
		{"PO", ignore}, /*Engraver command, not implemented, gives fake answer*/
 ...
 */
void SerialCommand::Init(void)
{
	term = '\r';
	commandCount = 16;
	defaultHandler = unrecognized;
...
void SerialCommand::readSerial() {
	while (RxBuf_NofElements() > 0) {
		...
		(void) RxBuf_Get(&inChar); // Read single available character, there may be more waiting
